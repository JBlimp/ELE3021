...
print_state(struct proc* p)
{
  ...
  if (p->state == UNUSED) {
    cprintf("my state: UNUSED\n");
  } else if (p->state == EMBRYO) {
    cprintf("my state: EMBRYO\n");
  } else if (p->state == SLEEPING) {
    cprintf("my state: SLEEPING\n");
  } else if (p->state == RUNNABLE) {
    cprintf("my state: RUNNABLE\n");
  } else if (p->state == RUNNING) {
    cprintf("my state: RUNNING\n");
  } else if (p->state == ZOMBIE) {
    cprintf("my state: ZOMBIE\n");
  }
  cprintf("I am in L%d\n", p->level);
  cprintf("My remaining quantum: %d\n", p->quantum);
...
// p가 현재 mlfq 큐에서 실행 가능하면 1 아니면 0 리턴
int
...
next_proc(int level)
{
  ...
  if (level >= 0 && level <= 2) {
    for (i = 0; i < NPROC; i++) {
      ...
      if (p->level == level && p->state == RUNNABLE) {
        queue_position[level] = (start + 1) % NPROC;
        return p;
      }
    }
  } else if (level == 3) {
...
scheduler(void)
{
  ...
  for(;;) {
    ...
    // 만약 스케줄링할 수 있는 프로세스가 없으면 continue
    if (i == 4) {
      continue;
    }
